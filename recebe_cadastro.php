<?php
if (!empty($_POST['metodo']) && !empty($_POST['nome']) && !empty($_POST['email']) && !empty($_POST['telefone']) && !empty($_POST['idade']) && !empty($_POST['genero']) && !empty($_POST['peso']) && !empty($_POST['altura']) && !empty($_POST['imc']) && !empty($_POST['estado']) && !empty($_POST['nivel-atividade']) && !empty($_POST['objetivo']) && !empty($_POST['tmb']) && !empty($_POST['tdee']) && !empty($_POST['consumo']) && !empty($_POST['proteina']) && !empty($_POST['gordura']) && !empty($_POST['carboidratos']) && !empty($_POST['doencas']) && !empty($_POST['medicamentos']) && !empty($_POST['problemas-intestinais']) && !empty($_POST['fumante']) && !empty($_POST['consome-alcool']) && !empty($_POST['objetivos']) && !empty($_POST['alergia-alimentar']) && !empty($_POST['triceps']) && !empty($_POST['subescapular']) && !empty($_POST['axilar-media']) && !empty($_POST['peitoral']) && !empty($_POST['suprailiaca']) && !empty($_POST['abdominal']) && !empty($_POST['coxa']) && !empty($_POST['percentualGordura'])) {
    $metodo = $_POST['metodo'];
    $nome = $_POST['nome'];
    $email = $_POST['email'];
    ...
    if ($conn) {
        ...
        $sql = "INSERT INTO nutri1 (id, metodo, nome, email, telefone, idade, genero, peso, altura, imc, estado, nivel_atividade, objetivo, tmb, tdee, consumo, proteina, gordura, carboidratos, doencas, medicamentos, problemas_intestinais, fumante, consome_alcool, objetivos, alergia_alimentar, triceps, subescapular, axilar, peitoral, suprailiaca, abdominal, coxa, percentualGordura) VALUES ('', '$metodo', '$nome', '$email', '$telefone', '$idade', '$genero', '$peso', '$altura', '$imc', '$estado', '$nivel_atividade', '$objetivo', '$tmb', '$tdee', '$consumo', '$proteina', '$gordura', '$carboidratos', '$doencas', '$medicamentos', '$problemas_intestinais', '$fumante', '$consome_alcool', '$objetivos', '$alergia_alimentar', '$triceps', '$subescapular', '$axilar', '$peitoral', '$suprailiaca', '$abdominal', '$coxa', '$percentualGordura')";
